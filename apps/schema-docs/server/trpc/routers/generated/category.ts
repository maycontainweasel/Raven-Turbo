...
import { Z_Category, RecordID_z } from '~~/app/types/schema/generated';

const CategoryCreateInput = Z_Category.partial().merge(
  Z_Category.pick({
  space: true,
  key: true,
  name: true,
...
const CategoryUpdateInput = z.union([
  z.object({
    id: z.union([z.string().min(1), z.number(), RecordID_z]),
    payload: Z_Category.omit({ id: true }).partial(),
  }),
  z.object({
    id: z.union([z.string().min(1), z.number(), RecordID_z]),
  }).merge(Z_Category.omit({ id: true }).partial()),
]);
...
const CategoryResourceKey = z.enum(['Admin']);
const CategoryResourceInput = z.object({
  id: z.union([z.string().min(1), z.number(), z.object({ tb: z.string(), id: z.any() }).passthrough()]),
  key: CategoryResourceKey.optional(),
...
export const categoryRouter = t.router({
  create: t.procedure
    .input(RequestSchema(CategoryCreateInput))
    .mutation(async ({ input, ctx }) => {
      ...
      if (!payload) {
        throw new Error('Category create payload is required');
      }
      const query = /* surql */ `
        RETURN fn::createCategory($payload);
      `;
      const result = await LRS(await dbInstance.query(query, { payload }));
      return result;
    }),
  update: t.procedure
    .input(RequestSchema(CategoryUpdateInput))
    .mutation(async ({ input, ctx }) => {
      const { db, LRS } = ctx;
      const dbInstance = input.instance && (ctx as any).$api?.DB
        ? await (ctx as any).$api.DB(input.instance as any)
      ...
      if (!id || !payload || Object.keys(payload).length === 0) {
        throw new Error('updateCategory requires an id and payload');
      }
      ...
      const query = /* surql */ `
        RETURN fn::updateCategory($id, $payload);
      `;
      const result = await LRS(await dbInstance.query(query, { id, payload }));
      return result;
    }),
  delete: t.procedure
    .input(RequestSchema(CategoryDeleteInput))
    .mutation(async ({ input, ctx }) => {
      const { db, LRS } = ctx;
      const dbInstance = input.instance && (ctx as any).$api?.DB
        ? await (ctx as any).$api.DB(input.instance as any)
      ...
      if (!id) {
        throw new Error('deleteCategory requires an id');
      }
      if (id && typeof id === 'object') {
        id = (id as any).id ?? (id as any).value ?? id;
      }
  ...
  resource: t.procedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (!id) {
        throw new Error('Category resource requires an id and resource key');
      }
      ...
      let resourceBaseModel: string = 'category';
      switch (resolvedKey) {
      case 'Admin':
        resourceView = "CategoryAdmin";
        resourceReturnId = 'record';
        resourceBaseModel = 'category';
        break;
      default:
      ...
      if (resourceView) {
        const selectIdClause = resourceReturnId === 'record'
          ? ',(type::record("category", record::id($this.id))) as id'
          : '';
        const query = /* surql */ `
      ...
      if (resourceFn) {
        const query = /* surql */ `
          LET $rid = type::record('category', $id);
