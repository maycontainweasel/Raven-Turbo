...
function getRuntimeAuth() {
  ...
  return { sessionSecret, cookieDomain, sessionCookie, isDev }
}
...
export async function setSessionCookie(event: H3Event, session: Session) {
  const { sessionSecret, cookieDomain, sessionCookie, isDev } = getRuntimeAuth()
  ...
  const sealed = await seal(crypto, session, sessionSecret, ironOpts)

  const opts = getCookieOptions(isDev, cookieDomain, defaultAuthConfig)
...
export async function readSession(event: H3Event): Promise<Session | null> {
  const { sessionSecret, sessionCookie } = getRuntimeAuth()

  try {
    const sealed = getCookie(event, sessionCookie)
    if (!sealed) return null

    const session = (await unseal(crypto, sealed, sessionSecret, ironOpts)) as Session

    // Refresh iat if older than configured refresh interval
    const now = Date.now()
    const sessionAge = now - session.iat
    const refreshThreshold = defaultAuthConfig.refreshInterval // e.g. 12 minutes
...
export function clearSessionCookie(event: H3Event) {
  const { cookieDomain, isDev, sessionCookie } = getRuntimeAuth()
  ...
  for (const name of cookieNames) {
    for (const path of defaultAuthConfig.appPaths) {
