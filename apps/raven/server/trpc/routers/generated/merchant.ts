...
import { Z_Merchant, RecordID_z } from '~~/app/types/schema/generated';

const MerchantCreateInput = Z_Merchant.partial().merge(
  Z_Merchant.pick({
  space: true,
  key: true,
  name: true,
...
const MerchantUpdateInput = z.union([
  z.object({
    id: z.union([z.string().min(1), z.number(), RecordID_z]),
    payload: Z_Merchant.omit({ id: true }).partial(),
  }),
  z.object({
    id: z.union([z.string().min(1), z.number(), RecordID_z]),
  }).merge(Z_Merchant.omit({ id: true }).partial()),
]);

const MerchantDeleteInput = z.object({
  id: z.union([z.string().min(1), z.number(), RecordID_z]),
});

const MerchantResourceKey = z.enum(['Admin']);
const MerchantResourceInput = z.object({
  id: z.union([z.string().min(1), z.number(), z.object({ tb: z.string(), id: z.any() }).passthrough()]),
  key: MerchantResourceKey.optional(),
...
export const merchantRouter = t.router({
  create: t.procedure
    .input(RequestSchema(MerchantCreateInput))
    .mutation(async ({ input, ctx }) => {
      ...
      if (!payload) {
        throw new Error('Merchant create payload is required');
      }
      const query = /* surql */ `
        RETURN fn::createMerchant($payload);
      `;
      const result = await LRS(await dbInstance.query(query, { payload }));
      return result;
    }),
  update: t.procedure
    .input(RequestSchema(MerchantUpdateInput))
    .mutation(async ({ input, ctx }) => {
      const { db, LRS } = ctx;
      const dbInstance = input.instance && (ctx as any).$api?.DB
        ? await (ctx as any).$api.DB(input.instance as any)
      ...
      if (!id || !payload || Object.keys(payload).length === 0) {
        throw new Error('updateMerchant requires an id and payload');
      }
      ...
      const query = /* surql */ `
        RETURN fn::updateMerchant($id, $payload);
      `;
      const result = await LRS(await dbInstance.query(query, { id, payload }));
      return result;
    }),
  delete: t.procedure
    .input(RequestSchema(MerchantDeleteInput))
    .mutation(async ({ input, ctx }) => {
      const { db, LRS } = ctx;
      const dbInstance = input.instance && (ctx as any).$api?.DB
        ? await (ctx as any).$api.DB(input.instance as any)
      ...
      if (!id) {
        throw new Error('deleteMerchant requires an id');
      }
      if (id && typeof id === 'object') {
        id = (id as any).id ?? (id as any).value ?? id;
      }
  ...
  resource: t.procedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (!id) {
        throw new Error('Merchant resource requires an id and resource key');
      }
      ...
      let resourceBaseModel: string = 'merchant';
      switch (resolvedKey) {
      case 'Admin':
        resourceView = "MerchantAdmin";
        resourceReturnId = 'record';
        resourceBaseModel = 'merchant';
        break;
      default:
      ...
      if (resourceView) {
        const selectIdClause = resourceReturnId === 'record'
          ? ',(type::record("merchant", record::id($this.id))) as id'
          : '';
        const query = /* surql */ `
      ...
      if (resourceFn) {
        const query = /* surql */ `
          LET $rid = type::record('merchant', $id);
