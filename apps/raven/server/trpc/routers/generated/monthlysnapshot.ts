...
import { Z_MonthlySnapshot, RecordID_z } from '@schema/types';

const MonthlySnapshotCreateInput = Z_MonthlySnapshot.partial().merge(
  Z_MonthlySnapshot.pick({
  space: true,
  month: true,
  currency: true,
  incomeTotal: true,
...
const MonthlySnapshotUpdateInput = z.union([
  z.object({
    id: z.union([z.string().min(1), z.number(), RecordID_z]),
    payload: Z_MonthlySnapshot.omit({ id: true }).partial(),
  }),
  z.object({
    id: z.union([z.string().min(1), z.number(), RecordID_z]),
  }).merge(Z_MonthlySnapshot.omit({ id: true }).partial()),
]);

const MonthlySnapshotDeleteInput = z.object({
  id: z.union([z.string().min(1), z.number(), RecordID_z]),
});

const MonthlySnapshotResourceKey = z.enum(['Admin']);
const MonthlySnapshotResourceInput = z.object({
  id: z.union([z.string().min(1), z.number(), z.object({ tb: z.string(), id: z.any() }).passthrough()]),
  key: MonthlySnapshotResourceKey.optional(),
...
export const monthlySnapshotRouter = t.router({
  create: t.procedure
    .input(RequestSchema(MonthlySnapshotCreateInput))
    .mutation(async ({ input, ctx }) => {
      ...
      if (!payload) {
        throw new Error('MonthlySnapshot create payload is required');
      }
      const query = /* surql */ `
        RETURN fn::createMonthlySnapshot($payload);
      `;
      const result = await LRS(await dbInstance.query(query, { payload }));
      return result;
    }),
  update: t.procedure
    .input(RequestSchema(MonthlySnapshotUpdateInput))
    .mutation(async ({ input, ctx }) => {
      const { db, LRS } = ctx;
      const dbInstance = input.instance && (ctx as any).$api?.DB
        ? await (ctx as any).$api.DB(input.instance as any)
      ...
      if (!id || !payload || Object.keys(payload).length === 0) {
        throw new Error('updateMonthlySnapshot requires an id and payload');
      }
      ...
      const query = /* surql */ `
        RETURN fn::updateMonthlySnapshot($id, $payload);
      `;
      const result = await LRS(await dbInstance.query(query, { id, payload }));
      return result;
    }),
  delete: t.procedure
    .input(RequestSchema(MonthlySnapshotDeleteInput))
    .mutation(async ({ input, ctx }) => {
      const { db, LRS } = ctx;
      const dbInstance = input.instance && (ctx as any).$api?.DB
        ? await (ctx as any).$api.DB(input.instance as any)
      ...
      if (!id) {
        throw new Error('deleteMonthlySnapshot requires an id');
      }
      if (id && typeof id === 'object') {
        id = (id as any).id ?? (id as any).value ?? id;
      }
      ...
      const query = /* surql */ `
        RETURN fn::deleteMonthlySnapshot($id);
      `;
      const result = await LRS(await dbInstance.query(query, { id }));
      return result;
    }),
  resource: t.procedure
    .input(RequestSchema(MonthlySnapshotResourceInput))
    .query(async ({ input, ctx }) => {
      ...
      if (!id) {
        throw new Error('MonthlySnapshot resource requires an id and resource key');
      }
      ...
      let resourceBaseModel: string = 'monthlySnapshot';
      switch (resolvedKey) {
      case 'Admin':
        resourceView = "MonthlySnapshotAdmin";
        resourceReturnId = 'record';
        resourceBaseModel = 'monthlySnapshot';
        break;
      default:
      ...
      if (resourceView) {
        const selectIdClause = resourceReturnId === 'record'
          ? ',(type::record("monthlySnapshot", record::id($this.id))) as id'
          : '';
        const query = /* surql */ `
      ...
      if (resourceFn) {
        const query = /* surql */ `
          LET $rid = type::record('monthlySnapshot', $id);
